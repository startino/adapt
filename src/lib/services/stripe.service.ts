...
export async function createCheckoutSessionServer(
	...
	try {
		...
		const customerId = await ensureCustomer(stripe, supabase, userId);
 ...
 */
export async function createCustomerPortalSessionServer(
	stripe: Stripe | null,
	supabase: SupabaseClient<Database>,
	userId: string,
	returnUrl?: string
...
export async function handleWebhookEvent(
	...
	switch (type) {
		...
		case 'customer.subscription.updated':
			...
			await updateSubscriptionStatus(stripe, supabase, subscription);
			break;

		case 'customer.subscription.deleted':
			const deletedSubscription = data.object as Stripe.Subscription;
			await deleteSubscription(stripe, supabase, deletedSubscription);
			break;
	}
